module.exports = function (app, db, nodemailer, io) {
            ...
            if (err) {
                if (err.code === "ER_DUP_ENTRY") {
                    if (err.message.includes("email")) {
                        return res
                            .status(400)
                            .json({ message: "Email đã được dùng cho tài khoản khác!" });
                    }
                    return res
                        .status(400)
                        .json({ message: "Tên đăng nhập này đã có người dùng!" });
                }
    ...
    app.post("/api/forgot-password", (req, res) => {
        ...
        db.query("SELECT * FROM users WHERE email = ?", [email], (err, users) => {
            ...
            if (users.length === 0)
                return res
                    .status(404)
                    .json({ error: "Email chưa đăng ký trong hệ thống!" });
            ...
            db.query(
                ...
                (updateErr) => {
                    if (updateErr)
                        return res
                            .status(500)
                            .json({ error: "Không thể cập nhật mật khẩu!" });
                    ...
                    transporter.sendMail(mailOptions, (mailErr, info) => {
                        if (mailErr) {
                            ...
                            return res
                                .status(500)
                                .json({ error: "Lỗi mạng, không thể gửi email!" });
                        }
    ...
    app.get("/employees", (req, res) => {
        const autoCheckoutSql = `
            UPDATE attendance
            SET check_out = CONCAT(work_date, ' 22:00:00'),
                shift_status = 'full',
                daily_wage = 300000
            WHERE shift_status = 'working' AND work_date < CURDATE()
        `;
        db.query(autoCheckoutSql, (autoErr) => {
            if (autoErr) console.error("Lỗi tự động chốt ca:", autoErr);
            db.query(
                "SELECT display_name, username, password, role, phone, status FROM users WHERE role != 'customer' AND status != 'inactive'",
                (err, results) => {
                    if (err) return res.status(500).json({ error: err.message });
                    res.json(results);
                },
    ...
    app.post("/employees", (req, res) => {
        ...
        db.query(
            "INSERT INTO users (display_name, username, password, email, phone, gender, date_of_birth, country, role) VALUES (?, ?, SHA2(?, 256), ?, ?, ?, ?, ?, ?)",
            [
                display_name,
                username,
                password,
            ...
            (err) => {
                if (err) {
                    ...
                    if (err.code === "ER_DUP_ENTRY" || errMsg.includes("duplicate")) {
                        return res
                            .status(400)
                            .json({ error: "Tên đăng nhập đã bị trùng!" });
                    }
                    return res
                        .status(500)
                        .json({ error: `Lỗi hệ thống: ${err.message}` });
                }
    ...
    app.put("/employees/:username", (req, res) => {
        ...
        db.query(
            "UPDATE users SET display_name=?, username=?, role=?, phone=?, status=? WHERE username=?",
            ...
            (err) => {
                if (err) {
                    ...
                    return res
                        .status(500)
                        .json({ error: `Lỗi cập nhật: ${err.message}` });
                }
    ...
    app.put("/api/employees/:username/reset-password", (req, res) => {
        const targetUsername = req.params.username;
        const defaultPassword = "12345678";

        db.query(
            "UPDATE users SET password = SHA2(?, 256) WHERE username = ?",
            [defaultPassword, targetUsername],
            (err) => {
                if (err)
                    return res
                        .status(500)
                        .json({ error: "Lỗi máy chủ khi cấp lại mật khẩu!" });
                res.json({ success: true, message: "Đã reset về mật khẩu mặc định." });
            },
        );
    });
    ...
    app.get("/api/users/:username", (req, res) => {
        db.query(
            "SELECT display_name, username, role, email, phone, gender, date_of_birth, country FROM users WHERE username = ?",
            [req.params.username],
            (err, results) => {
                if (err) return res.status(500).json({ error: err.message });
                res.json(results[0] || {});
    ...
    app.put("/api/users/:username", (req, res) => {
        ...
        if (new_password && new_password.trim() !== "") {
            db.query(
                "SELECT * FROM users WHERE username=? AND password=SHA2(?, 256)",
                [req.params.username, old_password],
                (err, results) => {
                    if (err) return res.status(500).json({ error: err.message });
                    if (results.length === 0)
                        return res
                            ...
                            .json({ error: "Mật khẩu cũ không chính xác!" });
                    db.query(
                        "UPDATE users SET display_name=?, email=?, phone=?, gender=?, date_of_birth=?, country=?, password=SHA2(?, 256) WHERE username=?",
                        [
                            display_name,
                            email,
                            phone,
            ...
            db.query(
                "UPDATE users SET display_name=?, email=?, phone=?, gender=?, date_of_birth=?, country=? WHERE username=?",
                [
                    display_name,
                    email,
                    phone,
    ...
    app.get("/api/attendance/:username", (req, res) => {
        ...
        db.query(
            "SELECT * FROM attendance WHERE username=? AND work_date=CURDATE()",
            [username],
            (err, todayRecord) => {
                ...
                db.query(sqlStats, [username], (err, stats) => {
                    db.query(
                        "SELECT * FROM attendance WHERE username=? ORDER BY work_date DESC LIMIT 10",
                        [username],
                        (err, history) => {
                            res.json({
                                ...
                                stats: stats
                                    ? stats[0]
                                    : { day_total: 0, week_total: 0, month_total: 0 },
                                history: history || [],
                            });
                        },
                    );
    ...
    app.post("/api/attendance/check-in", (req, res) => {
        db.query(
            "INSERT INTO attendance (username, work_date, check_in, shift_status) VALUES (?, CURDATE(), NOW(), 'working')",
            [req.body.username],
            (err) => {
                if (err)
                    return res
                        .status(400)
                        .json({ error: "Hôm nay nhân viên này đã được chấm công rồi!" });
                res.json({ success: true });
            },
        );
    });
    app.put("/api/attendance/check-out", (req, res) => {
        ...
        db.query(
            "UPDATE attendance SET check_out=NOW(), shift_status=?, daily_wage=? WHERE username=? AND work_date=CURDATE()",
            [shift_status, wage, username],
            (err) => {
                if (err) return res.status(500).json({ error: err.message });
                res.json({ success: true });
            },
